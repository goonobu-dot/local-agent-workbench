...
Do not include private logs, local vaults, or generated research output. If a contribution adds a generated workflow example, keep it fictional and run `make safety` before opening the pull request. Use `scripts/export_workflow.sh` when sharing a workflow folder outside your machine, and `scripts/import_workflow.sh` when reviewing a shared archive.
